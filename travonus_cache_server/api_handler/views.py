    ...
    def post(self, request, format=None, *args, **kwargs):
        ...
        if serialized_data.is_valid(raise_exception=True):
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=3) as executor:
                futures = [
                    executor.submit(bdfare_air_search, serialized_data.data),
                    executor.submit(sabre_air_search, serialized_data.data),
                    executor.submit(flyhub_air_search, serialized_data.data),
                ]
            ...
            if (
                ...
                query_str = f"@is_refundable:{{{request.GET.get('is_refundable')}}}"
            ...
            result = redis_client.ft().search(query)
            result = self.convert_to_json(result)

            # remove all json
            print("removed ", remove_all_flights())
